...
object PricingEngine {
    ...
    const val BASE_PACKAGING_FEE = 20.0
    ...
    fun calculatePackagingFee(orderType: OrderType): Double {
        return if (orderType == OrderType.TAKEAWAY) BASE_PACKAGING_FEE else 0.0
    }
    ...
    fun calculateTotal(subtotal: Double, deliveryFee: Double, packagingFee: Double, discount: Double): Double {
        val calculatedTax = calculateTax(subtotal)
        return (subtotal + deliveryFee + packagingFee + calculatedTax - discount).coerceAtLeast(0.0)
    }
}
